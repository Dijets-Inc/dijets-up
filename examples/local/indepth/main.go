...
func main() {
	...
	binaryPath := fmt.Sprintf("%s%s", goPath, "/src/github.com/lasthyphen/dijetsnodego/build/avalanchego")
...
func run(log logging.Logger, binaryPath string) error {
	...
	// Get one node
	dijetscub1, err := nw.GetNode(nodeNames[0])
	if err != nil {
		return err
	}

	// Get its node ID through its API and print it
	dijetscub1ID, _, err := dijetscub1.GetAPIClient().InfoAPI().GetNodeID(context.Background())
	if err != nil {
		return err
	}
	log.Info("one node's ID is", zap.Stringer("nodeID", dijetscub1ID))

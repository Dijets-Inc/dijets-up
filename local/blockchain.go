...
func (ln *localNetwork) installCustomChains(
	...
	baseWallet, djtxAssetID, testKeyAddr, err := setupWallet(ctx, clientURI, pTXs, ln.log)
	if err != nil {
		return nil, err
	}
	...
	ln.log.Info("base wallet DJTX balance", zap.Uint64("balance", balances[djtxAssetID]), zap.String("address", testKeyAddr.String()))
...
func setupWallet(
	...
	pTXs []ids.ID,
	log logging.Logger,
) (baseWallet primary.Wallet, djtxAssetID ids.ID, testKeyAddr ids.ShortID, err error) {
